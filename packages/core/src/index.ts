...
import traverseModule from "@babel/traverse"; const traverse: any = (traverseModule as any).default ?? (traverseModule as any);
...
/* ------------------------------------------------------------------ */
/*  Robust data-file resolution (works from CLI, workspace, or repo)  */
/* ------------------------------------------------------------------ */
...
function resolveDataFile(filename: string): string {
  const candidates = [
    ...
    // run from a workspace (e.g., packages/cli)
    path.resolve(process.cwd(), "..", "data", filename),
    // resolve from @flightdeck/core/dist to repo root
    path.resolve(__dirname, "..", "..", "..", "data", filename),
    // resolve from @flightdeck/core/dist to @flightdeck/core/data (if ever copied)
    path.resolve(__dirname, "..", "data", filename),
  ];
  ...
  // last resort: original process path (will throw later if missing)
  return path.resolve(process.cwd(), "data", filename);
}

const readJSON = (p: string) => JSON.parse(fs.readFileSync(p, "utf-8"));
const FEATURES: Record<string, FeatureMeta> = readJSON(resolveDataFile("features.json"));
const ALIASES: Record<string, string> = readJSON(resolveDataFile("aliases.json"));

/* ------------------------------ Config ----------------------------- */
function loadConfig(cwd = process.cwd()): FlightdeckConfig {
...
function estimateCoverage(
  ...
  let have = 0;
  for (const b of set) if (min[b]) have++;
...
function addHit(
  usages: Record<string, FeatureUsage>,
  id: string,
  file: string,
  line: number,
  column: number,
  ...
  u.count++;
  u.hits.push({ file, line, column, snippet });
  usages[id] = u;
}
...
function isProgressiveJS(context: string): boolean {
  return (
    /\bif\s*\(\s*['"][\w$]+['"]\s*in\s*[A-Za-z0-9_.]+\s*\)/.test(context) ||
    /try\s*{[\s\S]+}\s*catch\s*\(/.test(context)
  );
}
...
export async function scanProject(opts: {
  ...
  const analytics = parseAnalyticsCsv(
    cfg.analyticsCsv ? path.resolve(process.cwd(), cfg.analyticsCsv) : undefined
  );
  ...
  for (const file of files) {
    ...
    if (/\.(m?[jt]sx?)$/.test(file)) {
      ...
      traverse(ast, {
        enter(p: any) {
          ...
          if (p.isMemberExpression()) {
            const obj = p.node.object as any,
              prop = p.node.property as any;
            const text = `${obj?.name ?? (obj?.object?.name ? obj.object.name : "")}.${
              prop?.name ?? ""
            }`;
          ...
          if (p.isIdentifier()) {
            const id = ALIASES[(p.node as any).name];
            if (id) {
              const loc = p.node.loc?.start ?? { line: 1, column: 0 };
              const lineText = code.split("\n")[loc.line - 1]?.trim() ?? "";
              addHit(usages, id, file, loc.line, loc.column, lineText);
      ...
      const wrapped = hasSupportsWrapper(code);
      root.walkRules((rule) => {
      ...
      const doc = (parse5 as any).parse(code, { sourceCodeLocationInfo: true });
      const visit = (node: any) => {
        if (node.nodeName && ALIASES[node.nodeName]) {
          const loc =
            node.sourceCodeLocation?.startTag ??
            node.sourceCodeLocation ?? { startLine: 1, startCol: 0 };
  ...
  for (const [id, u] of Object.entries(usages)) {
    ...
    if (cfg.ignore.includes(id)) {
      u.severity = "info";
      continue;
    }
    if (feature.status === "none") u.severity = "error";
    else if (feature.status === "newly")
      u.severity = cfg.treatNewlyAsViolation ? "error" : u.severity === "info" ? "warn" : u.severity;

    if (cfg.overrides[id]?.severity) u.severity = cfg.overrides[id]!.severity!;
    if (cfg.overrides[id]?.minCoverage && u.coverage < cfg.overrides[id]!.minCoverage!)
      u.severity = "error";
  ...
  const violations = Object.values(usages)
    .filter((u) => u.severity === "error")
    .map((u) => u.id);
  const warnings = Object.values(usages)
    .filter((u) => u.severity === "warn")
    .map((u) => u.id);
  ...
  const res: ScanResult = {
    ...
    summary: {
      violations,
      warnings,
      pass: violations.length === 0 && achieved >= cfg.coverageBudget,
...
/* --------------------------- Report render ------------------------- */
// minimal HTML (add a severity column as needed)
function renderHtml(res: ScanResult): string {
  return `<!doctype html><html><body>
<h1>Baseline Flightdeck</h1>
<p>Violations: ${res.summary.violations.length} | Warnings: ${res.summary.warnings.length} | Coverage ${res.summary.achieved}% (budget ${res.summary.coverageBudget}%)</p>
</body></html>`;
}

function renderBadge(pass: boolean): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="130" height="20">
  <rect width="130" height="20" fill="#555"/>
  <rect x="60" width="70" height="20" fill="${pass ? "#4c1" : "#e05d44"}"/>
  <g fill="#fff" font-family="Verdana" font-size="11">
    <text x="7" y="14">baseline</text>
    <text x="70" y="14">${pass ? "passing" : "failing"}</text>
  </g>
</svg>`;
}
